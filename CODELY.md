...
- [2026-09-26 18:55:32] 【战斗地形无缝批五轮已落地（2026-09-26，会话 T，未提交——状态/HANDOFF 板会话 T 为准）】①几何缝：地块 mesh 0.96→边缘顶点扩满 1×1；②水面渲染缝：半透明分格必显缝→BattleBoard.BuildWaterSurface 整片焊接单 Mesh（角点焊接+世界 UV+「水面细分」默认 4）；③波浪动态缝=水格闭口盒墙顶穿出水面→TileWater.mesh 四壁/底面/顶面 submesh 全退役（0/0/2=只剩 0.30 湖床）；④贴片下潜=抬升低于波峰带→GetDecalHeight 贴片单出口；⑤底座圆盘被浪穿模（拍板「底座圆盘也改」）→**GetVisualSurfaceHeight 视觉表面高度单出口**（水/沼格=名义面+波峰带+0.01 余量；GetWaterWaveMaxHeight=实读水面材质 _WaveAmplitude，改 shader 波场公式须同步 1.5 峰值系数）——**CellToWorld/ContinuousCellToWorld（单位/投射物站位）与 GetDecalHeight 全走它；GetSurfaceHeight（拾取 §86/Host 口径）保持名义面勿把波峰带写进去**。**勿修清单：水块材质槽 1=水面材质载体、勿恢复分格顶面/格子墙、表面贴片高度走 GetDecalHeight/单位站位走 CellToWorld 勿手写 GetSurfaceHeight+常数、未来地图水贴虚空边界需 BuildWaterSurface 补裙边**。全案=docs/14 §89（五轮）。症状归因：地形格间缝/水面格界缝/波浪动态泥土线/贴片到水面下/水上单位被浪穿/水块空 submesh 报警先查本条与 §89。
