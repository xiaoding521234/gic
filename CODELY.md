...
- [2026-09-08 22:39:27] [project] 技能描述优化七角色已全部落地游戏（2026-09-08 完成，计划文件=.codely-cli/plans/技能描述优化-计划.md 留档）：①UnitConfig 落地丽莎5/琴6/诺艾尔6技能（customParams 按文档）+凯亚延奏补 Duration=3/StackLimit=5+霜袭补 MoraPlunder=5（文档有、早期落地遗漏，曾致游戏内裸{MoraPlunder}）；②SkillParamKey 新增 26 键（50-75：EnergyGain/PenaltyDamage/丽莎6/琴11/诺艾尔5/MoraPlunder）；③SkillName 枚举+表新增琴6(3011xxxx)/诺艾尔5(3012xxxx)；④SkillDescription 全部优化后描述五语言同步（含安柏3命 Concept:BaronBunny 修正、行秋契约简化版）；⑤RelatedName/RelatedDescription 补 5 Buff 条目（寒冰之棱/歌声之环/蔷薇之刺/蒲公英领域/雨帘剑，Key=中文裸名与链接 id 一致）；⑥Id 卫生：凯亚5+芭芭拉5 SkillDescription、SkillName WaterSerenade、SkillParamName 32 条历史错位全部 RemapId 对齐枚举值；⑦CSV 五表重导出；⑧三审计 PASS（占位符96处0缺失/三表Id全对齐/6种链接全可达）。**剩余**：13 张技能图标未生成（丽莎4/琴5/诺艾尔5+行秋4 亦缺，icon=fileID:0）、用户目检游戏内卡片。**How to apply:** 表直改一律走编辑器脚本幂等+AddKey 后 RemapId 三步法+两阶段重对齐模式（错位多条时先全去临时 Id 再落目标防碰撞）；新角色落地按此批流程。
- [2026-09-09 01:01:13] [project] 百分比架构优化落地（2026-09-09，行秋契约/诺艾尔1命引发的架构评审）：①评估结论=参数数据模型（typed key+baseType）已商业级、优于原神位置参数数组（gichess 旧 Java=行为接口无数据模型，网检业界=GAS 可缩放浮点+自定义计算类为重型方案）；②`SkillBaseType.Percent=11` 新增（上下文百分比：值=50 渲染"50%"，基数由技能语境提供）；③六参数 Fixed→Percent 转正（MoveSpeedReduction/PlunderRate/C1ShieldToEnergyRate/C1HealEfficiency/C1LifeSteal/C3DamageBonus）——凯亚1命/丽莎3命顺带修掉"治疗效率提升50"缺%的显示歧义；④**修实锤本地化缺陷：SkillDescriptionBuilder 基底名原硬编码中文（"攻击力"直注入所有语言描述），改走 StringDatabase.GetTable 当前语言读取**；⑤描述模板去手动 %（Percent 渲染自带，2键×5语言）；⑥SkillBaseType 表发现只建了 4 语言（缺 ru）——手动补建 ru 表全链（资产+Addressables String-Tables-Russian 组注册+12 值+预载对齐，磁盘已验证）；⑦文档 4 处参数表基准列+模板说明 Percent 行同步。**陷阱全沉淀在 gic-localization skill 新增"本引擎实测陷阱"节**（表 Id 禁 0/RemapId 单向/StringTable.Values/无 m_Tables+Addressables 注册机制/组名精确匹配/SetDirty 协议），勿在记忆重复。**遗留**：col.Tables 与 CSV 导出的 ru 列需编辑器刷新周期后才出现（磁盘已生效）；SkillBaseType 表 CSV 暂 4 语言列，下次会话重导出即可收 5 语言。**How to apply:** 新增百分比型技能参数一律用 Percent 基准；描述模板只写占位符不写 %；战斗系统 B 批次读 Percent 型按 rate×语境量结算。
