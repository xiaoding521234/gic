...
- [2026-09-23 22:09:16] 【B-S2 时轮编辑器首版已落地（2026-09-23「继续开发」指令，编译 0 错+反射自检过、待目检未提交）】`Editor/Tool/SkillTimelineEditor.cs`（Tools/TG/时轮编辑器，UI Toolkit+ConfigEditorUITK）：七轨时间轴（轨道色块+每轨「+」加 clip）+标尺（0.1/0.5s 刻度整数循环避浮点误差）+clip 交互（点击选中金框/水平拖拽平移 0.05s 吸附/右缘拖拽调时长）+按轨道属性面板（判定轨=SkillJudgmentKind+连发间隔+弹速直径射程+默认值提示/表现轨=cueName）+校验行（终点超总时长/无判定轨等）+「接线到 UnitConfig」按钮（按资产文件名匹配 skillID）+缩放滑条；编辑=直写对象+Undo.RecordObject（Ctrl+Z）+显式保存到磁盘。**Tuanjie UITK 新坑**：EnumField.newValue 装箱为 System.Enum——转 int 用 Convert.ToInt32 勿直接 cast（CS0030）。遗留=灰盒预览+scrub/SkillName 搜索下拉/动态总时长右界（docs/active/28 §7）。**下一步候选**：B-S3 素材路线待拍板（SFX/箭矢/AttackUp 图标：AI 生成 vs 官方提取 vs 效果库）——拍板后客户端表现轨消费（SkillCast 加载时轮资产播 cue）接线。
- [2026-09-23 22:29:42] 【技能配置独立化完成（2026-09-23 用户指令「先改造当前的技能配置。目前技能是内嵌在角色配置里，这非常不好」，编译 0 错+回读/运行链验证过、待目检未提交）】SkillConfig 独立资产化：每技能一个 SO（`Resources/Configs/Skills/{SkillName 枚举名}.asset`，data=SkillData：skillID/icon/skillType/customParams/timeline）；UnitData.skills=List&lt;SkillConfig&gt; 引用列表（顺序=skillIndex 语义勿重排、空槽告警）；迁移=A（一致性检查+生成 43 资产+保序 JSON）→类型变更→B（回填 12 角色 70 引用）；**Common_Walk 迁移决策**：8 角色两变体（带 MoveDistance 参数 vs 无参数）运行时等价（缺参默认=10%移速）→合并为带参数版；消费点改造 9 文件全走 `.data` 解引用（Unit/BattleHud×3/AIDebugBrain/BattleHeuristics×2/UnitDetailPanel/PetKnowledgeBaker/SkillTimelineEditor×2/UnitConfigEditor——UnitData 编辑窗选中技能=编辑 SkillConfig 资产本体+ping 按钮，通用技能共享资产改一处全角色生效）。验证：安柏 6 引用保序+参数/时轮接线无损+SkillFactory 运行链全对（4 技能类+3 占位）。gic-new-unit skill 同步流程已重写（建资产+引用，不再塞 UnitConfig）。
