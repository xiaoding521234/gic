...
- [2026-08-27 14:11:10] [feedback] Unity 改代码默认值≠运行生效：字段一旦序列化进场景/Prefab，场景值恒优先于代码默认值——"改了默认值构建后行为没变"时第一嫌疑就是场景旧值（2026-08-27 拖拽阈值 6→3 两轮构建无变化实证，写入场景序列化值后立即生效）。**Why:** Unity 反序列化规则：场景有值用场景值，代码默认值仅在字段首次序列化时进场景。**How to apply:** 改任何 Inspector 暴露字段的代码默认值时，同步用 SerializedObject 写场景值（或确认场景本无该字段）；排查"参数没生效"先查场景 YAML（中文键搜 \uXXXX 转义形式）。
- [2026-08-28 09:31:50] [feedback] execute_csharp_script 三坑（2026-08-27/28 实测，含一次 330s 超时+三次满核死循环）：①**项目有 CS 编译错误时严禁跑含 OpenScene 的脚本**——编译失败+场景加载叠加=编辑器主线程满核死循环（日志冻结+CPU 100%/10s，unity_dialog 查无对话框、只能杀进程重启）；铁律=批次间保持编译绿，动场景前先 start_compilation_pipeline 确认 0 错。②桥脚本编译器对 `new[]{...}` 集合表达式解析有 bug（CS9174/CS8361/CS1955），数组初始化一律 `new List<T>{...}` 或写 .cs 文件传路径执行（还须文件尾 `return X.Run();` 顶层语句）；顶层 yield 也非法（CS7020）——多帧等待须包局部函数返回 IEnumerator。③编辑器重启后桥心跳恢复慢（后台不刷新资产=吃不到新 .cs）——PowerShell ShowWindow(9)+SetForegroundWindow 前置编辑器窗口触发 AutoRefresh。④exec_runtime_script/exec_editor_script 在本大项目稳定性差：复杂脚本（FindObjectsOfType 大遍历+OpenScene+Play 组合）易挂死——脚本保持最小、分步跑；已验证可行的轻量模式=纯读场景 YAML/单组件 SerializedObject 检查。**How to apply:** 桥脚本失败/编辑器无响应时按此排查，勿盲目重试。 - [2026-08-28 09:30:00] [feedback] 【TMP_Dropdown.value setter 值不变也发回调】（2026-08-28 派蒙形态"点开栏目才切换"实证）：`Dd.value = x` 即使值与当前相同也 Invoke onValueChanged（uGUI/TMP Dropdown.Set 无同值短路）；恢复选中值一律 SetValueWithoutNotify。**放大器**：SettingsScreen 各分栏面板初始 inactive——LocalizedDropdown.Start（含 RefreshOptions）要到面板首次激活才跑，误触发被延迟到"用户点开该栏目"才发作，极难归因。**How to apply**：所有 dropdown 重建选项/恢复值路径改 WithoutNotify；"点开某个面板才发生 XX"类 bug 先查面板内组件 Start 延迟执行链。 - [2026-08-28 09:30:05] [feedback] 【DI 容器启动时序】（2026-08-28 启动形态分发实证）：Wargame.Init()（GameScene.Awake）只注册组件；存档加载在 SaveManager [PostConstruct]，到 GameScene.Start() 内 Wargame.Instance.Start() 才执行——**Awake 期读 CurrentSave 拿到的永远是 PlayerSaveData 默认值**。"启动时按存档初始化"的逻辑必须放 Wargame.Start() 之后。另：prefab 内"序列化字段指向桌面版专属组件"的控制器（PetBehaviorController.窗口控制器），守卫必须按运行时宿主/接口判空（宿主属性已做分发），不能按该字段判 null——游戏内形态该字段合法为 null，按它判空=整个行为层静默瘫痪（只剩待机动画）。
...
- [2026-08-28 00:23:25] [project] 游戏画面内版派蒙（2026-08-27，docs/19 §6.4）：批次 A+B 已完成并构建验证（设置第 6 栏/petForm 热切换/PaimonInGameRoot.prefab 走 Resources/IPetHost 宿主分发/PetPrefs 统一 pet.json v2/RT 画中画+UGUI 交互），提交至 e0655d8，**批次 B 尾段（PetPrefs/HostController）已构建验证未提交**——等用户目检热切换与画中画表现后收尾（docs/19 §6.4 落地记录+gic-pet skill 补节+提交）。风险点与实测清单见 .codely-cli/HANDOFF-游戏内派蒙.md。**How to apply:** 用户报目检结果后按 HANDOFF"已知待目检风险点"清单修；勿重做已完成部分。

- [2026-08-28 11:45:22] [project] 游戏内派蒙 v2（2026-08-28，docs/19 §6.4）：一轮复测三问题已修（见前）+二轮复测两问题已修：①全屏画布重构（RT=屏幕×rt倍率 1，RawImage 铺满+模型根在 RT 视野内移动，放大上限动态钳 0.95×H/h 不截断，交互全轮询 legacy Input+事件挡板=桌面 WS_EX_TRANSPARENT 同构，分辨率变化重建 RT）；②拖拽拎起姿势应用层复刻（转身 45°+横躺+挣扎+骨盆钉位=起手骨盆+光标位移，四肢摆动 LateUpdate 叠加；编辑器实测转身 44.9°、骨盆漂移 1.9% 屏高）。新构建 11:44 已启动（单进程=游戏内形态生效 ✓）——**等用户复测**（清单见 HANDOFF：放大不截/拖拽转身+Drag01+表情/挡板防穿透/失焦冻结边界）。**未提交**：全部改动（e0655d8 之后）。收尾=提交→docs/19 §6.4→gic-pet skill 补节→删 HANDOFF。
