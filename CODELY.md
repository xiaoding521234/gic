...
- [2026-08-30 12:27:03] [project] 派蒙聊天指令现状（2026-08-30）：仅 set_game_time/get_game_time 两工具（TimeUtility 偏移态+OnGameTimeChangedEvent 广播→大厅背景/位置音乐刷新）。**open_screen 界面切换指令 2026-08-30 用户拍板移除**（"没有必要"，手动切换足够快），代码已清（PetChatIntent 只剩时间工具+Execute 签名去掉 runner 参数；MainHallScreen 快速导航 API 全部回退）。教训留档：直接 Load 不藏大厅按钮=按钮透出叠穿（WishScreen 主 Canvas 是 ScreenSpaceCamera，大厅 Overlay 永远渲染其上）且可交互；若未来重做 AI 切界面，必须与手动 ExitToSceneAsync 同路径或隐藏态与退场动画结束态一致。**How to apply:** 勿再主动提议恢复 open_screen；后续指令（帮我抽卡/TTS）按 PetChatIntent 模式扩展；改指令语义先看 docs/19 §6.5。
