...
- [2026-08-27 23:03:58] [feedback] execute_csharp_script 三坑（2026-08-27 游戏内派蒙批次实测）：①**项目有 CS 编译错误时严禁跑含 OpenScene 的脚本**——编译失败+场景加载叠加=编辑器主线程满核死循环（日志冻结+CPU 100%），unity_dialog 查无对话框、只能杀进程重启；铁律=批次间保持编译绿，动场景前先 start_compilation_pipeline 确认 0 错。②桥脚本编译器对 `new[]{...}` 集合表达式解析有 bug（CS9174/CS8361/CS1955），数组初始化一律 `new List<T>{...}` 或写 .cs 文件传路径执行（还须文件尾 `return X.Run();` 顶层语句）。③编辑器重启后桥心跳恢复慢（后台不刷新资产=吃不到新 .cs）——PowerShell ShowWindow(9)+SetForegroundWindow 前置编辑器窗口触发 AutoRefresh，约 30-60s 后心跳恢复。**How to apply:** 桥脚本编译失败/编辑器无响应时按此排查，勿盲目重试。
...
- [2026-08-27 23:04:37] [project] 游戏画面内版派蒙进行中（2026-08-27，docs/19 §6.4）：批次 A 全完成（设置第 6 栏"派蒙"+petForm 字段+热切换链路+5 语言）、批次 B 半完成（PaimonInGameRoot.prefab+IPetHost 宿主分发已落地）；剩余=RT 画中画渲染+PetDragOrchestrator 抽取+UGUI 输入+持久化+构建目检。**状态快照与下一步清单见 .codely-cli/HANDOFF-游戏内派蒙.md（含两个待用户拍板项：prefab 走 Resources 还是 Addressables、悬浮位置持久化进主存档）**。边缘探头已砍（用户拍板）。**How to apply:** 续做此任务先读 HANDOFF；桌面版行为层已改宿主分发（IPetHost），动 PetWindowController/PetBehaviorController 前先看 HANDOFF 的场景结构侦察结论。
