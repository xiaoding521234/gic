...
- [2026-09-25 21:41:24] 【霜袭漂移修法 A+掠夺摩拉原子已落地（2026-09-25「继续」批=遗留清单按推荐推进，编译 0 错 0 警，复检清单已交，**未提交**——随下次「收尾」提交推送）】拍板=docs/18 决策九「B-3 首扩+修法 A」条，docs/11 两项销案。①修法 A：EffectCompiler.CompileJudgment 分流改「Burst 型 ∨ 配 LineBurst clip」都走整线迸发段编译——霜袭（Normal+LineBurst clip maxRange=2）回归设计=前方 2 格内敌人整线全中、瞬发无投射物视觉（白光条退役），预判/结算/HUD 瞄准三方对齐。②SkillEffectKind.MoraPlunder=7（B-3 首个资源类原子，护栏"不能组合表达才扩枚举"首触发）：OnHit 每命中一个敌人掠夺其**所属玩家**摩拉池→施法者玩家池（璃月契约先例同族=玩家池转移）；链路=MoraPlunderEffect 双玩家载荷（TargetUnitId=被掠夺玩家、ToPlayerId=掠夺方、AppliedGain=应用钳出实际量、HitSeconds）+ApplyEffects 实际量=min(参数, 被掠夺方池)（**池空抢不到=实现取值，观感确认点**）+双 StatChange(Mora) 命令按实际量（带命中时刻，客户端 PlayResourceDeltaCoroutine 到点发事件）+对账登记（AppliedGain=0 零命令=非漏发）+霜袭资产补配（paramKey=MoraPlunder 75 防双源）+B-2 编辑器 kind 7 显字段。How to apply：加资源类技能效果（掠夺/给予摩拉）=配 kind 7 原子零代码；契约类「不足掠夺钳 0」（docs/11 璃月条）复用同口径；霜袭复检要点=2 格内多敌全中无白光条、我方摩拉 +5/命中、AI 霜袭评分不变。
