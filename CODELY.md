...
- [2026-08-30 13:17:02] [project] 派蒙聊天指令现状（2026-08-30 下午更新）：set_game_time/get_game_time 两工具 + open_screen（同日重做恢复：AI 抽卡需切界面——导航铁律=弹层标准关闭回大厅→MainHallScreen.ExitToSceneAsync 同路径，PetGameBridge 隐藏协程宿主承载；联机/战斗互斥拒绝）+ auto_wish 自动抽卡（PetWishAutoRunner：导航→WishScreen.TryStartAutoDraw 自动射击→OnShotPlanned 逐发反应→OnWishComplete 结算）。**open_screen 移除拍板已被新需求推翻**：直接 Load 不藏大厅按钮=按钮透出叠穿（WishScreen 主 Canvas 是 ScreenSpaceCamera，大厅 Overlay 永远渲染其上）且可交互；AI 切界面必须与手动 ExitToSceneAsync 同路径。反应通道 PetReactionChannel（pet_react.jsonl 主进程→两形态宠物，文本/动作/LLM 注记三路，PetReactionConsumer 2.2s 节奏派发）。**How to apply:** 后续指令按 PetChatIntent 模式扩展；改指令语义先看 docs/19 §6.5；反应动作/阈值在 PetWishAutoRunner 一处定义。
