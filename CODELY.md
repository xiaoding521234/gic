...
- [2026-08-16 20:26:03] [2026-08-16 20:10] GIC 大地图 3D 化定稿：MapScreen = MapCamera（正交俯视，MapCameraController：抓取拖拽+乘法缩放[每格×0.85，范围8-15默认8]+clamp 边界+入场动画+射线点击）+ MapWorld/MapPlane（all_map=10752×6912/PPU50/DXT1 无 mip/71MB，世界 215×138）+ Anchor3D。**重大引擎发现（docs/14 §8.2 两轮定位）：Tuanjie 1.9.3 mipmapEnabled=true 的纹理块压缩静默失效回退未压缩格式（零警告）——项目内 map_back(无mip)=DXT1 而 nodkrai/mondstadt(有mip)=RGBA32 佐证，**全项目开了 mip 的贴图可能都在未压缩运行待专项排查**；显式格式覆盖（BC7/AutomaticCompressed）被无视，必须 textureFormat=-1 Automatic+Compressed；巨图(21504档)编辑器内重导入 OOM 崩溃须外部缩图（10752 档编辑器内安全）。**Why:** 3D 化平面观感+修加载卡顿+修缩放范围与清晰度。**How to apply:** 调缩放改场景 MapCamera 控制器字段；新贴图想压缩就别开 mip；详见 docs/13 §5、docs/14 §8。
