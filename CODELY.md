...
- [2026-09-01 21:30:08] [feedback] 【replace_all 子串陷阱】（三次实证）：① `_静默` replace_all 会把 `_静默淡出权重` 变成 `_silent淡出权重`（前缀包含）——改完必须搜残留混合名；② `坐线` 24 处替换把注释里的中文概念词也改了；③（2026-09-01 祈愿批）**加前缀类重命名的分步陷阱**：先单独改声明为 `_xxx`，再对旧名 `xxx` 做 replace_all——旧名是新名子串，已改好的声明被再改一遍成 `__xxx`（CS0103 15 处才暴露）。**How to apply**：批量 replace_all 前先确认无"以它为前缀/包含它的更长标识符"；补 `_` 前缀重命名必须声明+引用**一次性** replace_all（旧全名→新全名），绝不分两步；改完一批立刻编译，跨文件 rename 漏一处 CS0103 直接暴露。
...
- [2026-08-31 21:56:35] [project] 派蒙架构审查与治理进展（2026-08-31 深夜批 5-7 完成）：报告+执行状态表落 .codely-cli/HANDOFF-派蒙审查.md。批 1-4 已用户复测通过（4 个 P0 修复/死代码删除/全量英文化/序列化字段+pet.json v4→v5 迁移）。**批 5-7 代码完成待编译**（会话内 cron 每 2 分钟轮询编辑器退 Play 后自动编译+清 edgeSitCtrl 死键+构建）：批5=WireChat/WireReactionChannel/6 工具表/ConfigureTransparentCamera 下沉 PetHostBase（两宿主只剩 EnsureChatCanvas/ChatHeadAnchor/ChatFootAnchor/ExecuteChatTool 四钩子+behaviorCtrl/_chatUI 字段上移）；批6=IsSeated FindObjectOfType 缓存+PetEmotion 告警去重/OnDisable 复位+_history TrimHistory 封顶；批7=physicsEndFrame 拆三方法+P1-5 解耦（行为层删桌面专属 edgeSitCtrl 引用改 IPetHost.TrySnapAndSit/IsEdgeFalling，PetHostBase 虚默认 false 桌面 override）+FindBodyRenderer 静态化+两类头注释瘦身+docs/17 补 Pet 依赖方向行（Pet→Framework/Data/UI 单向，GameScene 组合根挂点豁免）。**How to apply**：批 7 大文件 partial 拆分未做（收益低）；方案B 工具家族已拍板保留不删（2026-08-31）；剩余拍板项=游戏内阴影门控/DevKey 吊销。GI 模型来源=models-resource（asset 328738，docs/19 §2.1）。
...
- [2026-09-02 00:21:29] [2026-09-02] [project] 移动端真机三问题已全部修复并经用户复测通过（2026-09-02"验证通过，提交"）：①画面压扁=移动端一律不调 Screen.SetResolution（Android resolutions 与横屏失配/空表回退 1920×1080 会被拉伸）；②双指被当右键关界面=InputManager 在 touchCount>0 时忽略鼠标键位绑定（触摸模拟第二指=Mouse1，勿全局关 simulateMouseWithTouches 因游戏内派蒙拖拽依赖它）；③游戏内派蒙不可见随①②修复后恢复（未单独立案）。陷阱全文 docs/14 §20/§21。同批：分辨率下拉重复（同尺寸不同刷新率）=SettingsApplier.GetUniqueResolutions 去重列表，设置下拉与 ApplyFromSave 共用（存档索引两端映射一致）；移动端分辨率下拉仅剩"全屏"单项（分辨率/窗口化是桌面概念）。**How to apply:** 移动端后续真机问题先查本条与 docs/14 §20/§21，勿重走弯路。
