...
- [2026-08-31 21:03:36] [feedback] 【replace_all 中文标识符的子串陷阱】（2026-08-31 批3 英文化实证两例）：① `_静默` replace_all 会把 `_静默淡出权重` 变成 `_silent淡出权重`（前缀包含）——改完必须搜残留混合名；② `坐线` 24 处替换把注释里的中文概念词也改了（可读性微降）。**How to apply**：中文标识符批量 replace_all 前先确认无"以它为前缀的更长标识符"；对注释中高频概念词（坐线/基准/平滑）用带上下文的精确 replace 而非 replace_all。另：改完一批必须立刻编译，跨文件 rename 漏一处会有 CS0103 直接暴露。
