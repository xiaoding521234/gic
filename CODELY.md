...
- [2026-08-14 23:33:38] - [2026-08-14 23:35:00] GIC DI 注入模式规范（P3a 确立）：①纯C#类构造注入 ②MonoBehaviour 场景单例（AudioManager/EventBusHub/NetworkEventBus）用**懒注入**——[Autowired] 字段+属性 getter 里 `if(null) Wargame.Instance?.Context?.Inject(this)`（因与 GameScene.Awake 顺序无保证）③运行时实例化的 prefab（Card）Awake 注入 ④静态门面（InputLocks）用后端注册制（InputManager [PostConstruct] RegisterBackend）⑤数据对象静态入口（CardConfigResolver.Instance，ConfigManager.Init 注册，镜像 StarVisualConfig 先例）⑥GameScene 是组合根，保留 Init/Start/Update/读存档直达。**Why:** P3a 去 Wargame.Instance 化确立的分级方案。**How to apply:** P3b UI 域及后续新代码按此分级选模式；manager 级 Wargame.Instance.Xxx 直达禁止新增。
