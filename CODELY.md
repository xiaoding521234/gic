...
- [2026-08-17 22:12:43] [2026-08-16] 大地图改原神式（用户明确要求对齐原神）：锚点全图常驻（SpawnAllAnchors）、区域按钮只动相机、区域切换平滑滑移（fromMaxZoom 区分初次打开）。**坐标系统=固定世界坐标系 + 垂直画布（2026-08-17 晚重构定稿并提交 0163533，Unity 2D 约定）**：地图平铺 XY 竖直平面（z=0），MapCamera/MapPlane rotation 全部 identity，相机朝 +Z 看画布（在 -Z 侧）；**关键迁移教训：垂直画布与旧俯视 XZ 轴向同向，z 值直接作 y（恒等迁移）——首轮误做 y=−z 镜像致锚点翻出图片、Play 黑屏**；AnchorData.world / viewCenterWorld 存绝对世界 XY 坐标；mapOrigin（图片左上角，(−107.52,69.12)）+ worldUnitsPerPixel（0.02）标定摆图（中心=origin+(w/2,−d/2,0)）；射线交互 z=0 求交（守卫=不平行且 t>0，勿按相机朝向假设方向符号——曾致拖拽缩放点击全失效）；Anchor3D.prefab collider center=(0,0,−0.7) 对准直立 icon。**工具已收敛（2026-08-17 夜，用户拍板删菜单）**：Tools/地图 只剩「坐标取点器」一项；MapCalibrationTool 为纯逻辑类（ApplyFromPickTool 单方法，无 MenuItem），应用标定按钮在取点器面板内（保存按钮下方）——工作流全在面板闭环：模式→取点→保存→应用→目测。取点器：4 模式（区域/锚点/新建/标定）；B 点硬约束投影吸附；恒等解自检=点绿圈中心 unit≈0.02；Undo 全链。区域按钮数据驱动（regionButtons）；锚点视觉大小 Anchor3D.prefab 根缩放 0.7。**Why:** 用户以原神+大厂实践为基准，工作流要低摩擦单入口。**How to apply:** 换图=覆盖 all_map.jpg→取点器标定模式点两下→保存→面板应用→目测；新代码勿再引入 XZ/Euler(90)/按朝向假设射线方向符号；后续标定工具改动只在取点器面板内做，不建菜单。
