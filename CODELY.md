...
- [2026-09-10 01:38:06] [project] 技能图标架构升级+安柏本地提取接线（2026-09-10 用户拍板）：**图标=裸图案（透明底），底板由 Skill.prefab 的 Badge 子级统一渲染**（badge.png=#253343、r=120 圆、勿再烘焙底板进图标；规范=docs/20 §2）。**元素染色**：SkillIconView 按 selfElement 运行时乘色（不改原图）；**元素颜色数值只存配置文件 Configs/ElementFactionConfig.asset**（原 ElementColor.cs 已退役删除；Instance.GetElementColor 静态访问=StarVisualConfig 同款单例模式）；ElementFactionIconConfig 已更名 **ElementFactionConfig**（用户拍板留扩容，后续元素/势力类配置都入此文件）。安柏 6 技能：#1 fly（通用飞行图标游戏内未定位）、#2 amber_double_shot（官方C1）、#3 arrow_rain（Skill_E_Ambor_HD）、#4 sharpshooter（官方C6）、#5 reveal、#6 constellation 通用。bow.png=通用射手战技图标（官方Skill_A_02，零引用留后续）。Badge 插入点=Skill.prefab child#0 + SkillDetailPanel 本地节点。超分=本地 Upscayl（upscayl-bin.exe digital-art-4x -r 256x256）；Fandom 弃用，官方素材一律本地提取（skill gic-gi-extract，归档=webrefs/genshin-unpack/extracted/ambor_skills/）。陷阱：安柏内名=Ambor 非 Anbi、图标 alpha×0.8 需×255/204、UI_Talent_U=E/Q 镜像、旧烘焙图与新裸图比对须多模态内容判定。
