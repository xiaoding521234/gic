...
- [2026-08-12 09:56:20] GIC 祈愿系统架构：WishPoolConfig(ScriptableObject) 定义卡池角色/物品/权重（star5Weight~star1Weight + unitWeight/itemWeight，概率=权重/总权重，无需凑满100） → WishManager 负责货币消耗+写入存档 → WishDrawController 控制 UI 动画。祈愿机制=反应速度游戏：卡道滚动出卡，玩家射击命中哪张卡获得哪张卡（不预抽），通过 Card.saveCardData 读取结果写入存档。卡池绑定：CharacterEntry.pool 直接引用卡池 asset，pool 为 null 则提示"卡池未开放"。卡池 asset 命名 WishPool_{势力英文名}.asset。当前权重：1/4/10/30/55（5★→1★），角色物品各50。星辉(Starglitter, ItemName=1006)为4★货币物品，祈愿可获得，初始数量0。重复角色卡转星辉（5★=50/4★=25/3★=15/2★=8/1★=3），count>0判重复，count=0为新获得。相遇之线：PlayerSaveData.starglitterEarned（累计获取量，只增不减，与可消费的星辉余额解耦）每满20触发1次，encounterUsed（持久化）记录已用次数。下次射击变金色相遇之线，射中卡先Roll提升次数（WishPoolConfig.RollUpgradeCount 使用独立升级权重 upgrade0Weight~upgrade4Weight，默认40:30:20:7:3，不再复用RollStarLevel），逐级星级提升。中间星级卡只做视觉变换+判重复发星辉（AwardDuplicateStarglitter，不写入存档），只有最终星级卡才AddFinalResultToInventory写入存档。相遇射击冷却固定(StarVisualConfig.BaseShotCooldown)不随星级变化，_isEncounterAnimating标志阻断射击+冻结冷却条直到动画完成，避免泄露提升次数。抖动+变亮动画强度和持续时间随星级递增（参数集中在StarVisualConfig）。动画完成后走HoldThenFly（与普通射击一致）。星辉雨用对象池+单协程批量管理（List<StarglitterDropData> struct）。**How to apply:** 新增卡池创建 WishPoolConfig asset 并拖到对应角色的 CharacterEntry.pool；UI 布局在场景 Canvas/WishDrawRoot 下改；货币显示在 WishScreen 根对象的 WishScreen（partial class WishScreenDraw）上，TopPanel/CurrencyContainer 下 PrimogemDisplay/StarglitterDisplay；WishDrawController 挂在 Canvas 上。
...
- [2026-08-13 10:13:25] GIC 大资源加载架构：大贴图/背景/立绘通过 Addressables 按需加载（Assets/Art/ 下 WishArt/{name}、PositionBack/{Region}/{name}、Map/Textures），Configs 和小 Prefabs 留在 Resources.Load。**所有 Addressables 加载/释放统一走 AssetCache**（Framework/AssetCache/AssetCache.cs），不再直接调用 Addressables.LoadAssetAsync。**How to apply:** 新增大贴图/背景/立绘放入 Assets/Art/ 对应目录并标记 Addressable；Configs 和小 Prefabs 留在 Resources；代码中用 `Wargame.Instance.AssetCache.LoadAsync<T>(address, callback, priority)` + `Release(address)` 替代直接 Addressables 调用。
...
- [2026-08-13 09:12:45] GIC DI 架构（2026-08-13 更新）：采用 Spring Boot 风格 IoC 容器。`ApplicationContext`（`Framework/DI/ApplicationContext.cs`）= Spring ApplicationContext。**注入分两种方式**：纯 C# [Component] 类用**构造器注入**（Register<T> 反射选最长构造函数解析依赖，字段标 readonly），MonoBehaviour 类用 [Autowired] 字段注入（Start()/Awake 调 Context.Inject(this)）。流程：阶段1 Register<ConfigManager>→ProcessConfigurations（拓扑排序+[Bean]产出，含循环依赖检测）→阶段2 按依赖顺序 Register Component（SaveManager→CardManager→PositionManager→PlayerManager→UnitManager→InputManager/UIManager/SkillManager）→InjectAll（MonoBehaviour 层）→Validate（缺失依赖抛异常）→PostConstruct。`[Configuration]`+`[Bean]`=ConfigManager 产出 UnitConfig/ItemConfig/PositionConfig/ElementFactionIconConfig/StarVisualConfig/CardConfigResolver。ConfigManager 自身仍用 [Autowired] 字段注入自身 [Bean] 产物（构造时不存在）。`Wargame.Context` 属性获取容器，`Context.Get<T>()`=getBean。**Why:** 用户要求高度模仿 Spring Boot，Spring 官方推荐构造器注入。**How to apply:** 新增纯 C# Manager 加 [Component]+构造器参数（在 Wargame.Init 阶段2 按依赖顺序 Register）；配置产出加 [Configuration]+[Bean]；MonoBehaviour 加 [Autowired] 字段+Context.Inject(this)；不要用 Wargame.Instance.XxxManager 取依赖。
- [2026-08-13 10:13:29] GIC AssetCache 缓存控制中枢（2026-08-13 新增）：AssetCache（Framework/AssetCache/AssetCache.cs）= [Component] 注册到 DI 容器，实现 IWargameManager。统一管理所有 Addressables 资源生命周期。核心机制：(1) 引用计数—LoadAsync 时 RefCount++，Release 时 RefCount--，归零且非 persistent 才真正释放 handle；(2) 去重—相同 address 并发请求合并为一次加载；(3) 帧预算—每帧最多 4ms 启动新加载，并发上限 8；(4) 优先级队列—LoadPriority.Low/Normal/High/Critical，高优插队；(5) Preload—标记 IsPersistent=true, RefCount=0，常驻不淘汰。Columbina 4K 立绘在 SplashScreen.Start() 中 Preload（比原来 MainHall.Start() 提前 ~2s），进入祈愿时已缓存。已删除旧 WishArtPreloader.cs。**How to apply:** 新增 Addressables 资源加载一律走 AssetCache.LoadAsync/Release；需要常驻的资源用 Preload；CharacterPanelController 和 MainHallScreen 背景加载均已改为 AssetCache。Addressables 分组：WishArt/PositionBack/MapAssets 各自 PackTogether，预加载首个资源即拉入整个 bundle。
- [2026-08-13 11:11:20] 已移除第三方包 com.besty.unity-skills (Unity-Skills 2.0.0, by Besty/GitHub Besty0728)。该包原通过本地路径引用 (file:D:/Ai/Unity-Skills-2.0.0/SkillsForUnity)，功能是在编辑器内嵌 HTTP Server 供外部 AI Agent 通过 REST API 操作 Unity。现统一使用 Codely Unity Bridge (cn.tuanjie.codely.bridge) 替代，无需 HTTP 中间层。**Why:** 用户改用团结 Codely 开发，Codely Bridge 提供原生 18 个 builtin tool 直接操作编辑器，功能完全覆盖且更高效。**How to apply:** 不再依赖 Unity-Skills 的任何 API（SkillsHttpServer/SkillsModeManager/UnitySkills 命名空间）；AI-Unity 集成全部走 Codely builtin tools。
