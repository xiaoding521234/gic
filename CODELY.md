...
- [2026-08-17 21:32:42] [2026-08-16] 大地图改原神式（用户明确要求对齐原神）：锚点全图常驻（SpawnAllAnchors）、区域按钮只动相机、区域切换平滑滑移（fromMaxZoom 区分初次打开）。**坐标系统=固定世界坐标系 + 垂直画布（2026-08-17 晚重构定稿，Unity 2D 约定）**：地图平铺 XY 竖直平面（z=0），MapCamera/MapPlane rotation 全部 identity，相机沿 -Z；AnchorData.world / RegionData.viewCenterWorld 存绝对世界 XY 坐标（**world.y=−旧z，世界 Y 向上=北**，MapConfig 存量已迁移）；MapConfig.mapOrigin（图片左上角，当前 (−107.52,−69.12)）+ worldUnitsPerPixel（0.02）标定摆图（ApplyMapCalibration：中心=origin+(w/2,−d/2,0)）；射线交互统一 z=0 平面求交；Y 翻转公式/bounds AABB 陷阱/SceneView 2D 冲突全部结构性消灭。**工具（Tools/地图/ 三件套）**：①MapCalibrationTool 应用标定（含 RegisterCompleteObjectUndo）；②MapPickPointTool 取点器（4 模式；Ctrl+点；标记=单环+20%实心圆法线 Vector3.back；5 视角对准 LookAtDirect identity；Undo 全链）；③两点标定（B 点硬约束投影吸附约束线，恒等解自检=点绿圈中心 unit≈0.02）。区域按钮数据驱动（regionButtons）；锚点视觉大小 Anchor3D.prefab 根缩放 0.7；MapAnchor icon 直立面向相机（悬浮偏移 -Z）。**Why:** 用户以原神为基准+成熟大厂编辑器实践（垂直画布是 Unity 2D 原生约定）。**How to apply:** 换图流程不变（覆盖 all_map.jpg→标定点两下→保存→应用→目测）；新代码勿再引入 XZ/Euler(90) 模式；待办：Play 模式实测整体观感+提交。
