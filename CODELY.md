...
- [2026-08-31 18:28:38] [project] 派蒙聊天指令现状（2026-08-31 用户复测通过）：set_game_time/get_game_time + open_screen（同路径导航：弹层标准关闭回大厅→MainHallScreen.ExitToSceneAsync，PetGameBridge 隐藏协程宿主承载；联机/战斗互斥拒绝）+ auto_wish 自动抽卡（PetWishAutoRunner：导航→EnsurePoolSelected 自动选绑卡池角色→TryStartAutoDraw 每发倒计时内随机时刻"手点"→OnShotPlanned 逐发反应→OnWishComplete 结算）。**关键坑：祈愿界面默认选中的首个角色（Columbina）未绑卡池，唯一卡池在 Venti——AI 抽卡必须先 EnsurePoolSelected 再等就绪，否则 IsAutoDrawReady 恒 false 超时**。反应通道 PetReactionChannel（pet_react.jsonl 主进程→两形态宠物，文本/动作/LLM 注记三路，PetReactionConsumer 2.2s 节奏派发）。同批：PopupDialog.prefab 断引用修复（迁移迟发漏网）+ toast 点击移出，均已复测通过。**How to apply:** 后续指令按 PetChatIntent 模式扩展；改指令语义先看 docs/19 §6.5；反应动作/阈值在 PetWishAutoRunner 一处定义。
