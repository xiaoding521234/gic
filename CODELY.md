...
- [2026-08-16 10:23:04] - [2026-08-15 21:29] 代码优化系列工程：docs/16-代码优化计划.md 为总纲（P1止血→…→P12 Tool整理+菜单栏整理），战斗核心(ActionQueue/DamagePipeline/Buff)不在计划内。**进度：全部完成收官——P12=6c97a79、追加批次 P12b（Config 工具按钮化入 Inspector+编辑器游戏字体）=35ad9df、docs=056c05a，均经用户验证（2026-08-16）。Why:** 用户要求分批优化全部代码防返工。**How to apply:** 优化计划已完结，后续新工作为战斗系统批次 B1-B4（落地时另行设计，ActionQueue+DamagePipeline 架构见另一条记忆）；不再读本计划取批次。
...
- [2026-08-15 21:44:19] GIC P11 Editor 配置编辑器已 UITK 重写完成并经用户验证（ListView 绑定增删/拖拽写回正常，编译零错误零警告）：公共层 Editor/ConfigEditorUITK.cs（CreateList：ListView bindingPath+根 Bind；ConfigInspectorBase；ElementDataEditorWindowBase；视觉组件：徽标按文本哈希 6 色盘取色/交替行/标题条/页脚主色保存按钮/危险红/SetBorderRadius——Tuanjie IStyle 无 borderRadius 简写）。Unit/Item/Position 三编辑器+窗口重写，ElementDrawer 双模式（OnGUI+CreatePropertyGUI），Editor 8 文件日志走 GICLog，删 SerializedPropertyContextMenu.cs。保存语义：窗口修改实时生效（Ctrl+Z 可撤销），「保存到磁盘」按钮落盘。**Why:** 用户选择 UITK 重写并授权美化，Editor 工具方向自此定为 UITK。**How to apply:** 新写 Editor 工具一律 UITK 并复用 ConfigEditorUITK；Tuanjie UITK API 差异见下条；提交时排除用户自改遗留 3 文件（WishPool_Mondstadt.asset / WishDrawController.Video.cs / mc_16x10.mp4）勿混提交。
...
- [2026-08-16 00:11:28] [2026-08-15] GIC P11 已完成并提交（5cbf73b，用户验证通过含 Sprite 预览；hash 已记入 docs/16 进度表，docs 提交 2af1779）。内容：Editor 全部窗口 UITK 化（3 Config 编辑器+4 工具窗口）+ ConfigEditorUITK 公共层 + Sprite 字段 IMGUI 预览行 + Editor 日志走 GICLog + 删 SerializedPropertyContextMenu.cs。工作区用户自改未提交新增：UnitConfig/ItemConfig.asset（用户用新编辑器测试保存的数据）、Assets/Video/ 目录（新增）。**How to apply:** 下批 P12（LightPillarEffect 拆分/WaitForSeconds 缓存等，交接见 docs/16 末节）；新写 Editor 工具一律 UITK + ConfigEditorUITK，不再写 IMGUI OnGUI。
...
- [2026-08-16 10:04:05] - [2026-08-16 10:15] GIC 编辑器工具规范（2026-08-16 确立）：①Config 类批量工具一律放对应 Config 的 Inspector 底部（ConfigInspectorBase.BuildTools / 独立 [CustomEditor] 底部工具区），不再建独立 EditorWindow 或 Tools 菜单项——已入 Inspector：UnitConfig（语音/图片）、ItemConfig（图标，路径 EditorPrefs 持久化）、ElementFactionIconConfig（图标）、WishPoolConfig（概率模拟，WishProbabilityTest.RunTest(pool) 测当前选中卡池）；Tools 菜单只留全局工具（场景/编辑器启动/Localization/存档/导出 APK/TMP/丢失脚本/URP）。②编辑器窗口字体统一走 ConfigEditorUITK.ApplyGameFont(root)（zh-cn.ttf=游戏 zh-cn SDF 源字体，根元素设一次全树继承，缺失静默回退）；新窗口/Inspector 必须调用。③Tuanjie UITK 陷阱：so.GetIterator() 上直接 GetEndProperty() 会 Assert "Invalid iteration"——根级遍历用 while(it.NextVisible(enterChildren)) 不配 end（元素级子属性用 element.Copy()+GetEndProperty() 则正常）。
- [2026-08-16 11:21:24] [project] Assets 清理最终状态（2026-08-16，含材质回滚）：保留已删——Assets/Editor 空目录、Scenes/TestScene.scene、StreamingAssets/mc.mp4（在用的是 mc_16x10.mp4）、Assets/AI/PRJ_SUMMARY.md、Assets 根 6 个 CUsers* 垃圾文件（文件名含 U+F03A 私有区字符）；cursor.png/icon.png 已移入 Resources/UI/Other/。**三个材质目录（Material/Materials/Resources/Materials 共 8 个材质）删除后已全部 git checkout HEAD 恢复**——"零引用"判断是文本审计误判（见 GUID 机制条目），其中 UIBlur/WishSmoke 确认在用（毛玻璃/祈愿烟雾），New Material 等其余 5 个真实性存疑但一并保留勿再删。保留：GlowTest.scene+CardGlowTest.cs。故意不做：NewAudioMixer 改名、Art/Wish 与 WishArt 改名（Addressables 地址约定）。**Why:** 删材质导致用户报告显示异常，已恢复并验证引用链。**How to apply:** 不再对这批材质做清理；图标移动触发的 SpriteAtlas 压缩格式提示为原有建议性警告。
...
- [2026-08-16 11:40:43] [project] _Scripts 结构重组第一批完成（2026-08-16，编译零错误）：①Framework/Test 两测试脚本→Editor/DevTest/（namespace 改 GIC.Editor，开发测试不入包）；②InputLocks/InputManager/KeyAction/KeyBindingEntry→Framework/Input/；③Singleton.cs 移出 Interfaces/ 到 Framework 根；Data/PlayerInfo/PlayerID/PlayerColor→Data/Player/；④docs/17"命名空间即目录"已修正为"namespace=六大模块根、目录=物理分组"（185 文件实际遵守后者），docs/15 InputLocks 路径已同步。移动全走 unity_asset move 保 GUID。**留待 B 批次**：Framework/Battle 4 个玩法 Manager（CardManager/CardDeck/SkillManager/UnitManager）与 UI/Screen/Wish/WishManager+FlowController 的层级归位，与 B3 管理器边界耦合。**Why:** 对照成熟项目分层审计后的 P2 一致性打磨。**How to apply:** 新文件按 docs/17 修正后约定放；不再单独做上述留待项。git 32 项变更待提交（含 .gitignore 补 .utmp/.gradle/keystore）。
