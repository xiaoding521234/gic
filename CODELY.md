...
- [2026-09-12 23:35:58] 【UI 结构改动交付必附目检清单】P2 面板化连续 4 回归实证（关闭残骸/入场瞬完成/闪现一帧/尖峰全屏闪烁，docs/14 §37+§38）：结构化断言全绿≠视觉正确——全部 4 个回归通过 36/17/12/11 项断言，均靠用户目检发现。**Why:** 断言盲区客观存在（组件级查不出 Canvas 残骸、时序类查不出渲染闪烁）。**How to apply:** P3 迁移 Backpack/Wish/Coop 及后续 UI 结构类改动交付时，主动列目检清单请用户验收（开有动画无闪烁/关彻底消失返回上级/反复开关正常/层级遮挡正确），不等用户撞上再修。
