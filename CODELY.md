...
- [2026-09-13 01:35:10] 手势输入层大重构 P1-P4 已拍板落档待实施（2026-09-13）：蓝图=docs/24-手势输入层重构技术设计.md（PointerInputPump 归一→GestureHub 三门+first-accept-wins 仲裁→纯 C# 8 态识别器 Tap/LongPress/Drag/Pinch；既有手感值一律保留为 surface 序列化覆写防回归；Win32 桌宠独立=桌面进程跳过 Wargame 的架构必然）。调研信源=.codely-cli/webrefs/gesture-input/（notes/gesture-system-research_2026-09-13.md，动手前先读）。**Why:** 指针手势碎片化——点击/拖拽判别手写 4 处两套、IsPointerOverUI 撞名 3 份、点击继续×4、鼠标/触屏双轨。**How to apply:** 实施以 docs/24 §6 阶段计划为准，每阶段一 commit+目检清单交用户；阶段完成更新 docs/17 手势层行。
