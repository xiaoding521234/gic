...
- [2026-09-10 01:15:37] [project] 技能图标架构升级+安柏本地提取接线（2026-09-10 用户拍板）：**图标=裸图案（透明底），底板由 Skill.prefab 的 Badge 子级统一渲染**（badge.png=#253343、r=120 圆、由旧图标 min-alpha 共识形状+平涂生成，勿再烘焙底板进图标；规范已入 docs/20 §2）。安柏 6 技能落地状态：#1 飞行冠军=fly（旧图保持，通用飞行图标游戏内未定位）、#2 一箭双丘丘=amber_double_shot（官方 C1 命座图，UnitConfig 已改接）、#3 箭雨=arrow_rain（同名内容替换 Skill_E_Ambor_HD）、#4 百发百中=sharpshooter（同名内容替换官方 C6 命座图——用户拍板用 C6 图案）、#5 全面侦查=reveal（旧图保持，官方无对应）、#6 命座=constellation 通用（拍板不换）。bow.png=通用射手战技图标（官方 Skill_A_02 裸图，当前零引用留给后续角色）。Badge 层插入点=Skill.prefab child#0 + SkillDetailPanel 本地 Skill 节点（嵌套实例自动继承）。内容替换法验证：bow/arrow_rain/sharpshooter 仅安柏自引用，零波及。超分工具拍板：图标放大一律本地 Upscayl（D:\Ai\Upscayl\resources\bin\upscayl-bin.exe，digital-art-4x，-r 256x256，alpha 自动保持）；同 artwork 多分辨率只留最高清源。提取归档=webrefs/genshin-unpack/extracted/ambor_skills/（256/=规范版）。Fandom 已弃用，官方素材一律本地游戏提取（skill gic-gi-extract）。关键陷阱：安柏内名=Ambor 非 Anbi、图标 alpha×0.8 需×255/204 修正、UI_Talent_U=E/Q 镜像副本、旧烘焙图与新裸图比对须多模态内容判定。
