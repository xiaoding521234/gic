...
- [2026-08-27 14:50:21] 桌宠构建/编辑器实测事实：①编辑器没开则桥死心跳（unity_refresh 拒连）——自救链=tuanjie.exe open 'D:\Tuanjie_editor\gic'（后台）→ Editor.log 出现 CompileScripts → unity_refresh 重连 → start_compilation_pipeline → 触发构建；残留无窗口 tuanjie 进程是许可/Hub 后台件，勿误判。②BuildPlayer 可成功覆盖正在运行的桌宠 exe（构建前不必杀旧进程，启动前必须杀）。③legacy Animation.Play() 播完的 Once clip 重播 time 正确归零从头播——排查切换类动画 bug 勿再疑"重播冻结在结尾姿势"此路。**How to apply:** 闭环遇心跳停先查进程再用 tuanjie-cli 拉起；构建协议细节见 gic-pet / gic-editor-longtask skill。
