...
- [2026-09-02 20:59:33] [2026-09-02 20:15:00] [feedback] 【.asset 外部编辑两陷阱】（2026-09-02 安柏对齐实证）：①**内存滞留**——编辑器运行中用 replace 外部改 .asset 后，LoadAssetAtPath 读回可能仍是旧对象：AssetDatabase.Refresh()/ImportAsset(ForceUpdate)/wait_for_idle 均未必触发重载；唯一可靠同步=内存对象直接改（如 customParams 的 key 字段）+ EditorUtility.SetDirty + AssetDatabase.SaveAssets 写穿（磁盘对、内存旧时此法一并同步，幂等安全）。②**技能参数双编号**——UnitConfig.asset 的 customParams `key: N` 用 SkillParamKey 枚举值（15=C2MoveSpeed、36=C2EnergyLimit、37=StackLimit），与 SkillParamName 本地化表 entry Id 是两套历史编号（表里 C2MoveSpeed=13）——改资产 key 前必查 SkillParamKey.cs 枚举值，勿拿本地化表 id 当资产键（实证：StackLimit 误写 key:15，15 实为 C2MoveSpeed，读回 GetInt(StackLimit)=-1 抓出）。**How to apply**：外部改 .asset 后一律编辑器读回断言（GetInt 默认值 -1 探测）；读回与磁盘不符时勿再信 Refresh，直接内存改+SaveAssets。（待迁 docs/14 技术陷阱）
...
- [2026-09-02 22:23:37] [2026-09-02 22:25] [feedback] 【AI 素材 9 切片两陷阱】（2026-09-02 弹窗素材实证，待迁 docs/14）：①Sliced sprite 的 border 按**原生像素**渲染、与显示矩形无关——端头弧线深的按钮素材（border 上下合计 94px > 显示高 65px）渲染必破碎，解法=Image.pixelsPerUnitMultiplier=2（2x 资产按 1x 比例渲染，线条仍清晰）；②spriteBorder 必须 ≥ 角部装饰/圆角半径，测法=各边"每行/列首个不透明像素缩进"的最大值+4px 余量（角饰超出 border 会被拉伸变形）。附：降采样用预乘 alpha 区域平均，防透明像素杂色 RGB（实测角落 RGB(71,112,76) a=0）渗成绿边；frontier-game-design is_segmentation=true 确有 alpha 通道，analyze_multimedia 把透明渲染成纯黑，勿误判为黑底图，用 C# 读像素核验。
...
- [2026-09-02 22:23:06] [2026-09-02 22:25] [project] 弹窗素材体系拆分定案（2026-09-02）：轻提示=ToastDialog.prefab（bottom.png 原横条，Boot.unity 的 PopupManager.toastPrefab 已指向）；中弹窗=PopupDialog.prefab（模态消息）+InputPopupDialog.prefab（输入框），面板/按钮素材在 Assets/Resources/UI/Popup/（medium_popup.png 9切片 border40 + popup_button_confirm/cancel.png，确认钮 mult=2 深字#3B4155）。**Why:** 原先模态与 toast 共用一个 prefab（toastPrefab 为空时兜底 popupPrefab），直接换中弹窗素材会连带改 toast 外观。**How to apply:** 改弹窗/轻提示外观前先分清两 prefab；弹窗类素材一律 9 切片。
