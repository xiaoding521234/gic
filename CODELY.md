...
- [2026-09-23 22:55:39] 【AnimLookTest muscle 目检载体重建（2026-09-23）】bake FBX 两份已落 Assets/Art/AmberAnimTest/（Ani_Avatar_Girl_Standby_bake.fbx=09-19 idle 版、Ani_Avatar_Girl_CrouchToStandby_bake.fbx=09-23 重验大动作版，**未提交，提交时记得带 .meta**）；四实例（AmberBake_Crouch x=0/AmberBake_Idle x=3.2/Amber_Harness_FBX x=-1.6/Odette x=1.6）clip 已全部导入器级 loop 化+组件 wrapMode=Loop（坑=docs/14 §73：AnimationState 层设置不序列化）；Idle 实例 phys clip 因 layer 无法序列化、无脚本不会自动播（预期非 bug）。**目检结论（2026-09-23 深夜）=不通过：效果与期望许多处不一致 → 用户拍板放弃自研管线，路线终局见同日终局条目+README 终局节；bake 实例+FBX 清理与否待拍板。**
