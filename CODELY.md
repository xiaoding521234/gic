...
- [2026-09-08 00:09:31] [project] 派蒙对话人设 v2.1 + 字体 tofu 根治（2026-09-07~08 收口）：人设=身份锚定"派蒙本人非AI"+九条说话规则+三句示例语料+显式禁特殊符号（模型会模仿 system prompt 符号风格，①「」 禁令保留——即使能渲染也破坏观感）。**tofu 根因当日纠错（初诊是错的）**：zh-cn.ttf（SDK_SC_Web，MD5 与原神安装 MiHoYoSDKRes\...\font\zh-cn.ttf 逐字节一致——用户从原神提取的真字体）文件级 cmap 口语字全有；真根因=zh-cn SDF 多图集关+单张 4096² 烘焙 1569 字已满，Dynamic 补字必败。**诊断铁律：TMP_FontAsset.HasCharacter=烘焙字形表，Font.HasCharacter=字体文件 cmap，两层勿混；Dynamic 字体资产建时必开多图集**。修复=开多图集+预烘 77 口语字（1646 字形/2 页）；**项目字体全量统一 zh-cn SDF（用户拍板，原神提取款）**：InputPopupDialog/CoopScreen 原 SourceHanSans 引用（m_fontAsset+m_sharedMaterial 双形态）已切至 zh-cn。改人设=代码+场景+prefab 三处同步（铁律 12）。细节勿重复：docs/14 §30、docs/19 §6.5.1。
