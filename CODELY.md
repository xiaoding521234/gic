...
- [2026-09-16 21:54:16] [2026-09-16] 【派蒙语音 TTS 设计拍板·终版（2026-09-16 深夜三轮收敛）】**三模式切换（设置→派蒙→语音区）：关闭 / 本地侧车 / 云端 TTS+克隆；零进包——游戏构建不塞任何 TTS 引擎/模型，Kokoro int8 进包方案已否（205MB 实测过但用户最终拍板玩家自配）**。侧车=玩家自己指定（地址+程序路径，默认 127.0.0.1:9880）；云端=玩家自填 key（MiniMax T2A v2+克隆，端点已验证）；音量滑条+开关要有；口型同步不做；覆盖=对话+抽卡反应+兜底全念（/指令回执不念）。**音色现状=社区模型（xianglun918/paimon-tts）耳检结论"接近派蒙但听得出不是派蒙"，用户接受此等级**；参考音频换活泼句（A 路线）可后续随时做、不阻塞代码。发布期议题=派蒙语音包可选组件安装器（4-6GB）或云 key，届时再拍。**Why:** 用户三连拍板——"能不能本地合成"→"社区看看"→"玩家开箱享受"→最终"不往游戏里塞任何包+玩家自配"， Kore 体积 vs 开箱的张力最终落在零进包。**How to apply:** P1a 已于 2026-09-16 同日全量交付（PetTtsClient+PetVoiceSpeaker+设置6行+pet.json 字段+本地化15键 id9081-9095+桌宠 AudioListener 兜底，编译 0 错，实现全档=docs/19 §6.5.11）；**待办=用户测试回报/桌宠测试包重建/云端模式无 key 未实测**；sherpa 进包彻底不做。
