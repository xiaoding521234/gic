...
- [2026-09-13 13:18:45] 【待补 docs/14 §46：程序化重组 UI 层级的两条 UGUI 陷阱】（2026-09-13 加载页逐像素填充落地实证）：①子节点 SetParent 进新容器时 anchor 参照系不迁移——原相对画布的锚（如 y=0.06"从底6%"）进 56px 高的层后变成"距层底 3.4px"整行挤出边界，迁移层级必须逐节点重设层内锚（y=0.5 居中等）；②单边锚点（锚父左缘）的动态宽度节点 pivot 必须设 (0,0.5)——默认 pivot 居中时宽度增长以中心为轴对称外扩，裁剪窗口一半跑出界外（实测 LitClip rect[-332,+332]）。另：RectMask2D 节点挂协程须先 SetActive(true) 再 StartCoroutine（inactive 对象协程起不来会报 Error）。**How to apply：** 下轮维护时把这三条整理进 docs/14 新小节；写程序化 prefab 重构脚本时默认带"迁移后逐节点重锚+pivot 复核"断言。 
