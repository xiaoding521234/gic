...
- [2026-09-27 01:50:56] 【B-S3 立牌动作段·视频路线已接线待目检（2026-09-27，未提交）】用户目检 Seedance 2.5 绿幕视频「效果非常满意」+拍板「不用序列帧（太占内存），使用透明底视频形式」——**立牌循环动画=绿幕 mp4+运行时 ChromaKey 抠色**（真 alpha WebM/VP9-YUVA 引擎支持矩阵混乱不押注；效果视频管线同款配方）：安柏=Assets/Art/PaperDoll/amber_fly_loop.mp4（Seedance 2.5 first_frame 生成 960²·5s·~1890 积分，参考=amber_glide 0.92 缩放压平纯绿 #00FF00；帧级验证最优循环段 [70,144)=74 帧裁剪 2.47s，回绕差分 0.72<相邻帧差 0.57~0.89=循环点不可察觉；baseline+bt709 VUI/容器双写）；实现=UnitData.立牌动画视频（VideoClip，优先级高于立牌动画帧，都缺=静态兜底）→UnitView 视频路径（VideoPlayer→RT ARGB32 sRGB 960²→ChromaKeyVideo shader 运行时抠色；阈值与离线管线同参=excess 平滑带 20~45/255+暗部门 g>60/255+despill；随机相位=vp.time；冻结/尸体 Pause 停摆保留当前帧；errorReceived 回落静态立牌 sprite；RT/材质 OnDestroy 释放，显存 ~3.7MB/单位恒定与帧数无关）；BattleViewFactory.CreateChromaKeyMaterial 唯一出口；shader 已登记 GraphicsSettings Always Included Shaders 防构建剥离；**三张序列帧 sheet 已从 Assets 删除**（tmp 存档：paperdoll_seedance/amber_fly_sheet_hd_archive.png + paperdoll_fly_anim/ 有 16/32 帧 sheet+分析工具链）。落档=docs/18 决策八「立牌动作段」条+active/28 §8+17 §7 速查/空缺+14 §90（桥脚本 VideoClip CS0246→LoadAssetAtPath&lt;Object&gt;+反射）。工具链=.codely-cli/tmp/paperdoll_seedance/（原片 amber_seedance.mp4+绿幕参考+extract_build.py 抽帧抠底循环检测+out/loop.gif 新旧对比图）。症状归因：安柏立牌不动→查 UnitConfig 立牌动画视频+OnVideoError 日志；安柏显示绿块→ChromaKey 阈值/shader 剥离（Always Included 清单）；其余单位静态=未配置属预期。旧序列帧接线条目（32 帧混合插值版）已随本路线作废；序列帧路径代码保留（其它单位可选帧或视频）。
