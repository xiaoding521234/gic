...
- [2026-08-15 16:49:01] GIC P10 已完成（2026-08-15，编译零错误+冒烟通过，待用户自测后提交）：①SkillDetailView 去克隆链路：字段 cardDetailViewTemplate→cardDetailView（BackpackScreen.unity 场景 CardModeContainer 下深拷贝主 CardDetailPanel 生成的嵌套实例，默认不激活），ShowCardMode 直接 Init(saveData)（幂等重置）②CardViewStrategyBase 抽出，Unit/ItemCardViewStrategy 改继承。**Why:** 消除每次点链接 Instantiate/Destroy 整面板的 GC 与手工重接线隐患。**How to apply:** 跨场景预设含场景级 override 子物体的面板必须 Object.Instantiate(场景实例) 深拷贝（prefab 源 InstantiatePrefab 实例不含场景 override）。**场景接线两条铁律（2026-08-15 修复 NPE 后确立）：** ①不要用 PrefabUtility.Set/GetPropertyModifications 往返清理字段——SetPropertyModifications 整体替换修改列表，会把同批刚赋值的新引用 mod 静默丢掉（曾致 cardDetailView 保存后变 null、点链接 NPE）；改用 SerializedObject.FindProperty().objectReferenceValue=xxx + ApplyModifiedPropertiesWithoutUndo()。②脚本改完场景必须"保存→CloseScene→重新 OpenScene→断言字段非 null"闭环，防止引用重映射静默失败。**编辑器安全红线：execute_csharp_script 禁止 LoadAssetAtPath 整个 prefab 后递归遍历层级（曾把主线程卡死 330s）。**
