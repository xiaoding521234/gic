...
- [2026-09-22 22:14:30] 【B6c 部署+手牌已落地待目检（2026-09-22 深夜，编译 0 错；docs/17 §7 增量行待目检通过后随提交补）】实现：①协议=ActionData 加 deployUnitName/deployCell、Segment 加 deploy 标记（部署段先于攻速分桶结算、占 sliceIndex 0 故攻速片 hasDeploySegment?1:0 起步防 ack 键撞）、PlayerResourceState 加 handUnits、BattleCommand 加 StatKindMora=2+summonUnit 字段+Summon 工厂；②SimState=RegisterHand（手牌=存档当前卡组投影、卡不消耗可重复出战）+GetHand+TrySpendMora/GetMora+GetCorePosition（核心位置代理=spawnCenters[PlayerIds 序]，**HashSet 迭代序坑：IndexOf 编译错改遍历计数；B8 协议核心 Unit 化时收口此处换真核心**）+SetPlayerTeam/GetTeamOf（装配时 P1=A/P2=B）+LogicRoot（部署生成单位挂隐藏根）；③DeployUnitExecutor（ActionExecutors.cs）=校验链（UnitConfig 有此角色→落点核心半径 2 内（DeployRadiusFromCore 常量+IsDeployCellValid，建筑半径判定待 DeployBuilding 批次）→摩拉够 GetEffectiveDeployCost）+扣费+UnitFactory 生成+直产命令（Summon 带 UnitState 全量+StatChange(Mora)；不走 BattleEffect 效应链、对账不涉及）；④TurnFlowController 白名单加 DeployUnit（unitId 空=玩家级行动豁免单位校验同 Pass）；⑤TurnResolver=部署段（deploys 先拆出、ResolveDeploySegment 逐条 TryResolve 产命令）；⑥客户端=BattlePlayer case Summon（CreateView+按布局态入位+RefreshAllFormations）+部署段 0.15s 短节拍不驱动高亮；⑦装配=BattleScreen SetPlayerTeam+RegisterHand（BuildHandFromCurrentDeck：注入 SaveManager 读 ownedCards 中 HasInDeck(currentDeck) 的 Unit 卡；空卡组回退丘丘人×2）+Sim.LogicRoot=logicGo；⑧HUD=RebuildHandCards（hand 布局件内横排卡按钮：深底+名+费用角标，卡面 polish 留 B6c-2）+EnterDeployAim（点卡→部署瞄准态，可选格=核心半径 2 客户端粗筛；FindMyCorePosition=spawnCenters[0] 本地 P1 惯例）+SubmitAim 部署分支（DeployUnit 上交占配额）+ExitAiming 部署分支（回 Idle 手牌态）。**待目检**：手牌显示/点卡高亮核心区/点格出战立牌出现/摩拉扣/重复出战/选中单位时手牌藏/摩拉不足与半径外落空 Log；**B6c-2 遗留**：手牌左右滑动（卡多溢出时）、正式卡面、Battle_TipAimDeploy 专属提示键（v1 复用方向瞄准提示）、物品卡/建筑卡、AI 出战决策。Why：中断归因。How to apply：部署类问题先查 DeployUnitExecutor 校验 Log 与手牌 RebuildHandCards。
