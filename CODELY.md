...
- [2026-08-29 23:14:32] [project] 【命名迁移数据丢失事故已修（2026-08-29）】d3149aa 标识符英文化迁移重保存场景/prefab 时丢失序列化值（仅 PaimonPet.unity + PaimonInGameRoot.prefab 受灾，其他场景 diff 均为死键清理无害）：①PetLookAtController 骨名×4（GI 英文名被冲回 MMD 日文默认值→头/眼球不跟踪鼠标，症状根因）②PetBehaviorController.edgeSitCtrl 引用丢失（桌面边坐失效）③liftEmotionName 用户手调值 Sleepy 被冲回代码默认 Confuse（拖拽表情不对）④PetInertializer.boneList 75 骨引用清空（惯性化关闭回退 CrossFade）。**Why:** 迁移用"改默认值+重保存"路径，凡场景值≠代码默认值的字段全被顶替。**How to apply:** ①排查"早期正常现在坏"类回归先做迁移前后场景值级 diff（.codely-cli/tmp/value_diff.ps1 按块比值序列）②迁移/批量改字段后必须 diff 值而不只看键名③PetLookAtController 默认骨名已改 GI 英文、PetSceneSyncTool/PetGIModelSetupTool 的中文 FindProperty 断裂已同步修复（编辑器工具引用运行时字段必须跟字段名走）。
- [2026-08-30 08:33:32] [project] 派蒙聊天首批指令已落地（2026-08-29 深夜，编译零错误，待用户目检）：①游戏内时间——TimeUtility 加偏移态（默认=系统时间，进程内静态不持久化=每次启动自动重置；SetGameTime/ResetToSystemTime/Now）+ OnGameTimeChangedEvent 广播（LocalEvents.cs）→ MainHallScreen 订阅刷背景（同时段同地址跳过）+ PositionManager 订阅换曲（**同时段调时间不打断当前曲**——_currentMusicPeriod 守卫）。②open_screen（map/backpack/wish/settings）——PetChatIntent.cs（Chat 目录，Intent 层：工具定义+执行）；**导航规则=平级弹层先 ScreenBase.Close() 标准关闭（退场动画+音乐恢复+GoBack 回大厅）→ 等历史栈清空+SceneTransition 锁释放 → 再 Load，绝不叠层**；CoopScreen 房间状态机排除（打开时报"请先手动退出"）；执行前先 CloseChat() 收输入条释放输入锁。③get_game_time 查询。**Why:** 用户要求"设置游戏内时间（初始=系统时间）+打开指定界面（界面地图一致，不能叠层）"。**How to apply:** 工具仅游戏内形态注册（PetInGameHostController.接聊天，桌面形态独立进程 IPC 未建不注册）；改时间链路先看 docs/19 §6.5 首批指令节；后续指令（帮我抽卡/TTS）按 PetChatIntent 模式扩展。
