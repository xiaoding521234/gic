...
namespace GIC.Pet
{
    ...
    public class PetBehaviorController : MonoBehaviour
    {
        ...
        // 边坐控制器字段已删（2026-08-31 批 7 解耦：桌面专属类型不走共用行为层，经 IPetHost.TrySnapAndSit/IsEdgeFalling 分发）
        [InspectorName("拎起动作名")]
        ...
        [Header("拖拽dragReaction（2026-08-27：按时长分档）")]
        ...
        private string _currentOneShotName;
        ...
        private bool _exitDone;              // 退场动画播完（回调已发）：冻结一切行为保持退场末帧，直到进程退出/实例销毁
        private System.Action _exitDoneCallback; // 退场动画播完执行（进程关闭，由窗口控制器注入）
        private bool _ceremonySilent;            // 出场/退场动画期间：眨眼+视线层静默（收尾统一解除）
        private bool _liftGazeSilent;        // 拖拽物理期视线静默（2026-08-26：拎起 KO 垂落头被视线层拉向光标=目检"抬头看抓取点"根因）
        private bool _liftEmotionOn;          // 拖拽物理期的拎起表情状态（边沿检测用）
        private bool _physicsLastFrame;             // 物理交互结束边沿：回待机/清拎起表情

        /// <summary>当前待机动作：坐定中=坐姿（行为层视角的"待机"随坐定切换），
        /// 否则=站立待机。打招呼/小动作/dragReaction播完都回到这里——坐定期间穿插的单次动作自然回坐姿。
        ...
        /// 经 IPetHost 分发（游戏内形态无掉落概念恒 false，2026-08-31 批 7 解耦）。</summary>
        private bool edgeSitFalling => host != null && host.IsEdgeFalling;
        ...
        public bool IsExiting => _exiting || _exitDone;
        ...
        void Update()
        {
            ...
            if (_exiting)
            {
                if (Time.time - _oneShotStart > 0.25f && !animPlayer.IsPlaying(_currentOneShotName))
                {
                    _exiting = false;
                    _exitDone = true;
                    var cb = _exitDoneCallback;
                    _exitDoneCallback = null;
                    cb?.Invoke();
                }
                return;
            }
            ...
            if (_physicsLastFrame) physicsEndFrame();

            // 出场：首帧（全部 Start 已完成 = PetAnimSwapper 预热已回待机）播出场动画；
            // 播完走下方 _oneShotPlaying 分支自然回待机；无出场动画则直接进正常待机逻辑。
            ...
            if (proximity && !edgeSitFalling && !IsSeated && Time.time - _lastGreetTime >= greetCooldownSec)
            {
                _proximityTimer += Time.deltaTime;
                if (_proximityTimer >= triggerHoldSec)
                {
                    PlayOneShot(greetAnimName);
        ...
        /// 沿革：v1 程序化四肢垂落叠加层（PetDanglePoseController，已删除 2026-08-31）→ v2 Sleep01
        ...
        void dragPhysicsPhase()
        {
            if (_oneShotPlaying)
            {
                ...
                if (_ceremonySilent) setCeremonySilent(false); // 出场动画被打断
            ...
            // SetSilence(false) 内部有 _刚恢复 重同步，头会平滑回到视线跟随而非瞬移
            if (!_liftGazeSilent)
            {
                _liftGazeSilent = true;
                lookAtCtrl?.SetSilence(true);
            }
            if (!_liftEmotionOn)
            {
                _liftEmotionOn = true;
            ...
            _physicsLastFrame = true;
        }

        /// <summary>物理刚结束帧：清拎起表情+视线恢复 → dragReaction分档 / 边坐接管 / 回待机</summary>
        void physicsEndFrame()
        {
            EndLiftOverlays();
            string dragReaction = PickDragReaction(host.DragSeconds);
            if (!string.IsNullOrEmpty(dragReaction))
                PlayOneShot(dragReaction);
            else if (host.TrySnapAndSit())
            {
                // 桌面版边坐接管：松手位置附近有横框（窗口顶边/底边/任务栏顶），已磁吸落座播坐姿动画
                //（2026-08-31 批 7 解耦：原直接引用桌面专属 PetEdgeSitController，现经 IPetHost 分发——
                // 游戏内形态恒 false，其屏幕坐已在宿主物理收口里评估）
            }
            else if (!IsSeated && !_oneShotPlaying && animPlayer.HasAnim(currentIdleAnim))
            {
        ...
        /// <summary>解除拎起期覆盖层：视线恢复（内部重同步平滑回跟随）+ 表情清空</summary>
        void EndLiftOverlays()
        {
            _physicsLastFrame = false;
            if (_liftGazeSilent)
            {
                _liftGazeSilent = false;
                lookAtCtrl?.SetSilence(false); // 内部重同步平滑基准，头平滑回到视线跟随
            }
            if (_liftEmotionOn)
            {
                _liftEmotionOn = false;
                if (emotionCtrl != null && !_oneShotPlaying) emotionCtrl.SetEmotion("");
            }
        }

        /// <summary>拖拽放下反应分档（2026-08-27 拍板）：短拖（&lt;轻阈值）无反应直接回待机
        /// （快速挪位置不打扰）；中档（≥轻阈值）害羞；长拖（≥生气阈值）生气。
        /// PlayOneShot 含情绪映射（Anger→生气表情/Shy→害羞）+动作期烘焙暂停，播完自然回待机。</summary>
        string PickDragReaction(float draggedSec)
        {
            if (draggedSec >= dragAngerThresholdSec && animPlayer.HasAnim(dragAngerReactAnim))
                return dragAngerReactAnim;
            if (draggedSec >= dragLightThresholdSec && animPlayer.HasAnim(dragLightReactAnim))
                return dragLightReactAnim;
            return null;
        }
        ...
        void oneShotFrame()
        {
            if (Time.time - _oneShotStart > 0.25f)
            {
                bool finished = !animPlayer.IsPlaying(_currentOneShotName);
                ...
                float rem = animPlayer.RemainingSeconds(_currentOneShotName);
                bool tailBlend = !animPlayer.IsInertializationOn && !finished && !_ceremonySilent && rem > 0f && rem <= animPlayer.TransitionSeconds * 0.9f;
                if (finished || tailBlend)
                {
                    _oneShotPlaying = false;
                    if (_ceremonySilent) setCeremonySilent(false); // 仪式（出场）静默解除；普通单次动作本来就没静默视线
                    else blinkCtrl?.SetSilence(false);
                    host.PauseHitBaking = false;
                    animPlayer.Play(currentIdleAnim);
                }
        ...
        void PlayOneShot(string 动作名)
        {
            _currentOneShotName = 动作名;
            _oneShotPlaying = true;
            _oneShotStart = Time.time;
            blinkCtrl?.SetSilence(true);
        ...
        {
            _ceremonySilent = silent;
            blinkCtrl?.SetSilence(silent);
            lookAtCtrl?.SetSilence(silent);
        }
        ...
        bool detectCursorProximity()
        {
            ...
            if (!host.TryGetCursorUnityScreenPos(out Vector2 sp)) return false;

            Bounds b;
            if (host.TryGetHitWorldBounds(out b))
            {
                // 命中网格路径（首选）：真实蒙皮世界包围盒
            }
