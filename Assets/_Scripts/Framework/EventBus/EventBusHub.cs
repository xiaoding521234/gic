...
namespace GIC.Framework
{
    ...
    public class EventBusHub : MonoBehaviour
    {
        ...
        public void Publish(BaseEvent eventData)
        {
            ...
            {
                GICLog.Warn("[EventBusHub] SendEvent: 事件无效或未激活");
                return;
            }
            ...
            switch (eventData.Type)
            {
                case EventType.Local:
                    if (_enableDebugLog)
                        GICLog.Info($"[EventBusHub] SendEvent: 本地事件 Type={eventData.GetType().Name}, SourcePlayerID={eventData.SourcePlayerID}");
                ...
                case EventType.OnlyHost:
                    if (_enableDebugLog)
                        GICLog.Info($"[EventBusHub] SendEvent: 网络事件 Type={eventData.GetType().Name}, EventType={eventData.Type}, SourcePlayerID={eventData.SourcePlayerID}");
                    ...
                    {
                        GICLog.Error("[EventBusHub] RouteToNetwork: NetworkEventBus 未找到");
                    }
                    break;

                default:
                    GICLog.Warn($"[EventBusHub] SendEvent: 未知事件类型 Type={eventData.GetType().Name}, EventType={eventData.Type}");
                    break;
            }
        }
        ...
        public void PublishToPlayer(string targetPlayerID, BaseEvent eventData)
        {
            if (eventData == null || !eventData.Active)
            {
                GICLog.Warn("[EventBusHub] SendEventToPlayer: 事件无效或未激活");
                return;
            }

            if (_networkEventBus == null)
            {
                GICLog.Error("[EventBusHub] SendEventToPlayer: NetworkEventBus 未找到");
                return;
            }
            ...
            GICLog.Info($"[EventBusHub] SendEventToPlayer: TargetID={targetPlayerID}, Type={eventData.GetType().Name}, SourcePlayerID={eventData.SourcePlayerID}");
        ...
        public void PublishToPlayers(System.Collections.Generic.List<string> targetPlayerIDs, BaseEvent eventData)
        {
            if (eventData == null || !eventData.Active)
            {
                GICLog.Warn("[EventBusHub] SendEventToPlayers: 事件无效或未激活");
                return;
            }

            if (_networkEventBus == null)
            {
                GICLog.Error("[EventBusHub] SendEventToPlayers: NetworkEventBus 未找到");
                return;
            }
            ...
            GICLog.Info($"[EventBusHub] SendEventToPlayers: Targets=[{string.Join(",", targetPlayerIDs)}], Type={eventData.GetType().Name}, SourcePlayerID={eventData.SourcePlayerID}");
        ...
        public void ReceiveNetworkEvent(BaseEvent eventData)
        {
            if (eventData == null || !eventData.Active)
            {
                GICLog.Warn("[EventBusHub] ReceiveNetworkEvent: 事件无效或未激活");
                return;
            }

            eventData.Source = EventSource.Network;

            GICLog.Info($"[EventBusHub] ReceiveNetworkEvent: Type={eventData.GetType().Name}, SourcePlayerID={eventData.SourcePlayerID}, Immediate={eventData.Immediate}");
